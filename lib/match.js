...
// Role effectif d'un joueur : le role assigne manuellement (form) sinon le role
// principal deduit de l'historique.
function effRole(p, roleMap) {
  return roleMap?.[p.riotId] || p.mainRole;
}

// Joueur d'une equipe a un poste : priorite a l'assignation manuelle, sinon le
// 1er joueur dont le role principal est `role`.
function playerAtRole(team, role, roleMap) {
  return (
    team.players.find((p) => roleMap?.[p.riotId] === role) ||
    team.players.find((p) => p.mainRole === role) ||
    null
  );
}
...
function laneMatchups(mine, enemy, myRoles, enemyRoles) {
  return ROLES.map((role) => {
    const a = playerAtRole(mine, role, myRoles);
    const b = playerAtRole(enemy, role, enemyRoles);
...
function draft(team, roleMap) {
  return ROLES.map((role) => {
    const picks = [];
    for (const p of team.players) {
      if (effRole(p, roleMap) !== role) continue;
...
export function analyzeMatch(mine, enemy, meta, opts = {}) {
  const { myRoles, enemyRoles } = opts;
  const idx = metaIndex(meta);
  const pool = myPoolByRole(mine);
  const enemyDraft = draft(enemy, enemyRoles);
  ...
  return {
    laneMatchups: laneMatchups(mine, enemy, myRoles, enemyRoles),
    enemyDraft,
    myDraft: draft(mine, myRoles),
    bansAgainst,
    pickSuggestions,
  };
}
