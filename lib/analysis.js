...
function championStrength(champ) {
  ...
  const volumeComponent = champ.playRate; // volume global sur la saison
  const recentComponent = champ.recentPlayRate; // volume sur les 30 dernieres
  const winComponent = champ.winrate;
  const kdaComponent = Math.min(champ.kda.ratio / 5, 1); // KDA 5 = excellent
  const raw =
    0.4 * volumeComponent +
    0.25 * recentComponent +
    0.25 * winComponent +
    0.1 * kdaComponent;
...
export function analyzeTeam(playerRaws, ddragon, meta) {
  ...
  for (const [championId, entries] of champToPlayers) {
    ...
    const score = Math.min(
      100,
      (best.strength * flexFactor * metaFactor + masteryBoost) * 100
    );
