...
function aggregatePlayer(games, masteries, ddragon, recentWindow) {
  const masteryById = {};
  ...
  // recentWindow premieres servent au playrate recent.
  games.forEach((g, idx) => {
    ...
    c.games++;
    if (idx < recentWindow) c.recentGames++;
    if (g.win) c.wins++;
    c.kills += g.kills;
    c.deaths += g.deaths;
    c.assists += g.assists;
  ...
  const recentTotal = Math.min(totalGames, recentWindow);
...
function championStrength(champ, S) {
  ...
  const recent = champ.recentPlayRate; // volume sur la fenetre recente
  const win = champ.winrate;
  const kda = Math.min(champ.kda.ratio / 5, 1); // KDA 5 = excellent
  // Bonus de maitrise : desormais une composante de la force (donc multipliee
  // ensuite par les facteurs meta/flex/niveau), plutot qu'un ajout final.
  const mastery = champ.masteryPoints >= S.masteryThreshold ? S.masteryBoost : 0;
  const raw = S.wVolume * volume + S.wRecent * recent + S.wWin * win + S.wKda * kda + mastery;
  return { value: confidence * raw, confidence, volume, recent, win, kda, mastery };
}
...
export function analyzeTeam(playerRaws, ddragon, meta, settings) {
  ...
  const players = playerRaws.map((pr) => {
    const games = pr.matches
      ...
      .filter(Boolean);
    const agg = aggregatePlayer(games, pr.masteries, ddragon, S.recentWindow);
  ...
  for (const [championId, entries] of champToPlayers) {
    ...
    for (const e of entries) {
      const s = championStrength(e.champ, S);
    ...
    const score = Math.min(
      100,
      best.s.value * gameFlexFactor * metaFactor * skillFactor * 100
    );
    ...
    bans.push({
      ...
      breakdown: {
        ...
        weights: { wVolume: S.wVolume, wRecent: S.wRecent, wWin: S.wWin, wKda: S.wKda },
        force: best.s.value,
        gameFlexFactor,
        roleCount,
        metaFactor,
