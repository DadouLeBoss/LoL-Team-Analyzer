...
export function analyzeTeam(playerRaws, ddragon, meta) {
  ...
  const players = playerRaws.map((pr) => {
    ...
    // Les 10 parties les plus récentes (l'ordre Riot est déjà anti-chronologique).
    const recentMatches = games.slice(0, 10).map((g) => ({
      championId: g.championId,
    ...
    // Les 10 champions avec le plus de points de maîtrise.
  ...
  for (const [championId, entries] of champToPlayers) {
    ...
    const score = Math.min(
      100,
      (best.s.value * gameFlexFactor * metaFactor + masteryBoost) * 100
    );
