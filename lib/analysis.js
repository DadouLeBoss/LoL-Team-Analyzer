...
export function analyzeTeam(playerRaws, ddragon, meta) {
  ...
  const players = playerRaws.map((pr) => {
    ...
    // Les 6 parties les plus récentes (l'ordre Riot est déjà anti-chronologique).
    const recentMatches = games.slice(0, 6).map((g) => ({
      championId: g.championId,
    ...
    // Les 6 champions avec le plus de points de maîtrise.
