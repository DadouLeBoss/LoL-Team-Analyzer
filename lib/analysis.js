...
export function analyzeTeam(playerRaws, ddragon, meta, settings) {
  ...
  // calcul de l'impact differentiel du ban (force - repli du 2e meilleur choix
  // du meme joueur dans le meme role) et evite de recalculer les forces.
  ...
  for (const [championId, entries] of champToPlayers) {
    ...
    // Impact differentiel : on retranche une part du 2e meilleur champion du
    // meme joueur dans le meme role. Pool profond -> ban peu utile (l'adversaire
    // a un repli equivalent) ; one-trick sans repli -> impact ~ egal a la force.
    // Exception : un pick "en preparation" n'est pas un repli interchangeable
    // (choix delibere, souvent un contre pour le match) -> pas de retranchement.
    ...
    const repli = isPrep ? 0 : S.banDropoff * secondBestForce;
    const effectiveForce = Math.max(0, best.s.value - repli);
    ...
    const score = Math.min(
      100,
      effectiveForce * gameFlexFactor * metaFactor * skillFactor * prepFactor * 100
    );
