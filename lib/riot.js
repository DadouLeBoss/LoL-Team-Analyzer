...
// Ne conserve que les champs utiles a l'analyse (~3 Ko au lieu de ~75 Ko par
...
//
// En plus des champs de base (KDA, role, victoire), on capte des indicateurs
// bruts (CS, degats aux champions, or, vision, camp, niveau) pour de futures
// analyses (CS/min, degats/min, winrate par side, duos...). Ils ne sont pas
// encore exploites par analyzeTeam mais sont stockes des maintenant : le format
// est versionne (MATCH_CACHE_VERSION), donc l'historique se reconstruit une fois
// avec ces donnees puis reste en cache.
export function slimMatch(match) {
  ...
  return {
    ...
    participants: match.info.participants.map((p) => ({
      ...
      teamId: p.teamId, // 100 (bleu) ou 200 (rouge)
